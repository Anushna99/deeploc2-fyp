...
def generate_embeddings(model_attrs: ModelAttributes):
    ...
    '''    
    if(model_attrs.dataset == 'swissprot' and model_attrs.model_type == FAST):
        embedding_source_file = EMBEDDINGS["Fast"]["source_fasta"]
        embedding_file = EMBEDDINGS["Fast"]["embeds"]
    if(model_attrs.dataset == 'swissprot' and model_attrs.model_type == ACCURATE):
        embedding_source_file = EMBEDDINGS["Accurate"]["source_fasta"]
        embedding_file = EMBEDDINGS["Accurate"]["embeds"]
    if(model_attrs.dataset == 'hpa' and model_attrs.model_type == FAST):
        embedding_source_file = EMBEDDINGS["Test_esm"]["source_fasta"]
        embedding_file = EMBEDDINGS["Test_esm"]["embeds"]
    if(model_attrs.dataset == 'hpa' and model_attrs.model_type == ACCURATE):
        embedding_source_file = EMBEDDINGS["Test_prott5"]["source_fasta"]
        embedding_file = EMBEDDINGS["Test_prott5"]["embeds"]
    
    # Example usage of embedding_type (or return it if needed)
    print(f"Embedding source selected: {embedding_source_file}, Embedding file selected: {embedding_file}")

    fasta_dict = read_fasta(embedding_source_file)
    # store the fasta readings to a csv file
    fasta_format = os.path.splitext(os.path.basename(embedding_file))[0]

    print(f"saving fasta format of: {fasta_format} to a csv.")
    ...
    if model_attrs.model_type == FAST:
        ...
        embed_esm1b(embed_dataloader, embedding_file)
        ...
        embed_prott5(embed_dataloader, embedding_file)
    else:
