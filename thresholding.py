...
# Load your CSV file
df = pd.read_csv('thresholding_input_file.csv')
...
df_test.to_csv('thresholding_test_set.csv', index=False)
...
def calculate_cost(tau, w1=1.0, w2=1.0):
    ...
    FN = ((true_labels == True) & rejected).sum()
    FP = ((true_labels == False) & accepted).sum()
    return w1 * FP + w2 * FN
